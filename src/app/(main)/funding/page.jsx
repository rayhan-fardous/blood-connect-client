...
import { HandCoins, Loader2, DollarSign, Sparkles, X, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  if (sessionLoading || fetchLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 gap-3">
        ...
        <p className="text-sm font-medium text-slate-500">Loading platform details...</p>
      </div>
    );
  }
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              ...
              <p className="text-slate-600 mt-2 text-sm md:text-base max-w-md leading-relaxed">
                Your donations help us cover server costs, send urgent blood alert SMS messages, and keep our platform free for everyone.
              </p>
            </div>
            <button
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xs">
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-50 font-medium text-slate-700">
                {paginated.map((fund) => (
                  <tr key={fund._id} className="hover:bg-slate-50/50 transition">
                    <td className="px-4 py-4 text-slate-900 font-bold">{fund.donorName}</td>
                    <td className="px-4 py-4 text-emerald-600 font-semibold">${fund.amount.toLocaleString()}</td>
                ...
                {paginated.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-4 py-12 text-center text-slate-400 font-normal">
          ...
          <div className="md:hidden space-y-3">
            {paginated.map((fund) => (
              ...
              >
                <div>
                  <p className="font-bold text-slate-900 text-sm">{fund.donorName}</p>
                  <p className="text-xs text-slate-400 mt-0.5">
                    {new Date(fund.date).toLocaleDateString('en-BD')}
                  </p>
                </div>
                <p className="text-emerald-600 font-bold text-base">${fund.amount}</p>
              </div>
            ))}
            {paginated.length === 0 && (
            ...
            <div className="bg-white w-full max-w-md rounded-3xl border border-slate-100 shadow-xl relative overflow-hidden p-6 md:p-8">
              ...
              <div className="space-y-5">
                <p className="text-sm text-slate-600 leading-relaxed">
                  Every dollar goes directly into upgrading platform security, server stability, and improving donor matching alerts.
                </p>
